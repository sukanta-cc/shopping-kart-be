...
module.exports = {
  ...
  addProduct: (req) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { name, description, productCode, featured, amount } = req.body;

        const imageUrls = await Promise.all(req.files.map((item) => item.path));
  ...
  changeProductStatus: (req) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        return resolve({
          success: true,
          message: messages.products.PRODUCT_STATUS_UPDATED_SUCCESS.replace(
            "{{status}}",
  ...
  getFeaturedProducts: (req) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        return resolve({
          success: true,
          message: messages.products.FEATURED_PRODUCTS_FETCHED_SUCCESS,
          result: products,
        });
      } catch (error) {
